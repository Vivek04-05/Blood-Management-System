...
public class AddRecevier extends JFrame implements ActionListener ,ItemListener,WindowListener{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddRecevier frame = new AddRecevier();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(name.isEmpty() ||email.isEmpty() ||phone.isEmpty() ||age.isEmpty() || bloodgroup.isEmpty() || bag.isEmpty() || address.isEmpty() || gender_type.equalsIgnoreCase(" ")) {
		...
		try {
			...
			String str="insert into bloodrequestdetails  (PhoneNo, NoOfBags, BloodGroup, Date) values(?,?,?,?)";
			ps=con.prepareStatement(strsql);
			ps.setString(1, name);
			...
			// for str  Table
						...
						ps1.setInt(2, b);
						ps1.setDate(4, sd);
						ps1.setString(3, bloodgroup);
			
			int sts=ps.executeUpdate();
			int stat1=ps1.executeUpdate();
			if(sts>0 && stat1>0) {
				
				JOptionPane.showMessageDialog(this, "Recevier Data add successfully");
			}
		}
		catch(SQLException se) {
			se.printStackTrace();
